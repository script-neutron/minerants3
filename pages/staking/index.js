...
export default function Staking() {
    ...
    if (typeof window != "undefined"){
        ...
        const stakingContract = new ethers.Contract(stakingAddress, stakingContractAbi, provider)
    ...
    useEffect(() => {
        ...
        async function fetchdata(){
            ...
            if (wallet !== undefined) {
              ...
              minamount = (ethers.utils.formatUnits(minamount._hex,0)/1e18).toFixed(2)
              ...
              amountstaked = (ethers.utils.formatUnits(amountstaked._hex,0)/1e18).toFixed(2)
              let rewardsamount = tx.total_rewards
              rewardsamount = (ethers.utils.formatUnits(rewardsamount._hex,0)/1e18).toFixed(2)
      ...
      async function stake(){
        ...
        document.getElementsByClassName('myinfo')[0].innerHTML = 'Approving MANT ...'
        infotoggle()
        ...
        tx.wait(1).then(async ()=>{
          ...
          document.getElementsByClassName('myinfo')[0].innerHTML = 'Minimum MANT required for staking is 50000.00 MANT'
          infotoggle()
          console.log(err)
        })
      }
      ...
      async function Withdrawstakes(){
        ...
        tx.wait(1).then((rez)=>{
          ...
          document.getElementsByClassName('myinfo')[0].innerHTML = 'minimum staking period isnt up'
          infotoggle()
        }
  ...
  return (
    <div>
        <Mininav/>
        <h1 className='font-bold text-2xl text-pink-500 text-center m-4 p-4 '>Minimum MANT required for staking is {stakeamount} MANT</h1>
            <div className='mx-8 flex justify-center m-7 text-center gap-4'>
            ...
            <div className='mx-8 flex justify-center m-7 p-4 text-center'>
                <div className='border-2 border-rose-500 rounded-lg p-4 m-4 text-center'>
                    <ul>
                        <li className='font-medium text-lg text-white p-2'><h1>Enter Stake Amount</h1></li>
                        <li className='font-medium text-lg p-2'><input type="number" className='stake rounded-lg text-center' /> </li>
                        <li className='p-2'> <button className='btn' id='sbtn'  onClick={stake}> Confirm Stake</button></li>
                    </ul>
                </div>
                <div className='border-2 border-rose-500 rounded-lg p-4 m-4 text-center'>
                    <ul>
                        <li className='font-medium text-lg text-white p-2'><h1>Withdraw Rewards</h1></li>
                        <li className='font-medium text-lg text-white p-2'><input type="number" className='rounded-lg text-center' value={rewards} disabled /> </li>
                        <li className='p-2 wbtn'> <button className='btn' id='wbtn' onClick={Withdraw}> Withdraw</button></li>
                    </ul>
                </div>
                <div className='border-2 border-rose-500 rounded-lg p-4 m-4 text-center'>
                    <ul>
                        <li className='font-medium text-lg text-white p-2'><h1>End Staking</h1></li>
                        <li className='font-medium text-lg text-white p-2'><input type="number" className='rounded-lg text-center' value={stakewithdraw} disabled /> </li>
                        <li className='p-2'> <button className='btn' id='wbtn'  onClick={Withdrawstakes}> Withdraw</button></li>
                    </ul>
                </div>
            </div>
