...
export default function HatchEgg() {
    ...
    async function fetchdata() {
      ...
      if (wallet !== undefined) {
        ...
        document.getElementsByClassName('myinfo')[0].innerHTML =
          'Loading your ants...<br> Please be patience';
        ...
        document.getElementsByClassName('claimed')[0].innerHTML =
          claimed.toFixed(2);
        let nft = [];
        let workers = 0;
        for (let i = 0; i < myants.length; i++) {
          ...
          if (nftowner === wallet.toLowerCase()) {
            ...
            if (URI.attributes[0].value != 0) {
              ...
              workers++;
              console.log(workers);
              document.getElementsByClassName('unclaimed')[0].innerHTML =
                unclaimed.toFixed(2);
  ...
  const listitems = myArray.map((item) => {
    ...
    return (
      <div
        className={`hatch-cards ${item.id} text-center m-4 p-4 shadow-lg shadow-indigo-500/50 rounded-lg`}
        key={item.id}
      >
        <h1 className="font-medium text-lg text-cyan-600">
          {' '}
          Unmined rewards:{' '}
        </h1>
        <h1
          className={`${
            'anttoken' + item.id
          } font-bold text-2xl text-orange-600 m-1`}
        >
          ----
        </h1>
        <Image
          ...
          height="500px"
          alt="img"
          className={`${'img' + item.id} rounded-lg`}
        />
        <h5 className={`${'id' + item.id} font-medium text-lg text-red-300`}>
          {item.id}
        </h5>
        <h1
          className={`${
            'description' + item.id
          } font-medium text-lg text-cyan-300`}
        >
          {' '}
          {item.name}
        </h1>
        ...
        <h3
          className={`${'attack' + item.id} font-medium text-lg text-pink-600`}
        >
          {' '}
          Attack Pow: {item.attributes[1].value}
        </h3>
        <h3 className={`${'date' + item.id} font-medium text-lg text-pink-600`}>
          {' '}
          Birthday: {date}
        </h3>
        <button
    ...
    <div>
      ...
      <div className="p-4 m-4 container mx-auto">
        <h1 className="text-lg font-bold text-center text-pink-600">
          {' '}
          View all Your Miner Ants Eggs here and Hatch them
        </h1>
