...
function NestPage(){
    ...
    return (
        <>
        {data.map((item) => (
        <div className='flex text-center justify-around p-8 m-8 border-2 border-rose-500 rounded-2xl' key={item.zodiac}>
            <div className='flex flex-col'>
                <div className='flex justify-center py-3'>
                <h3 className='btn text-lg font-bold p-4 m-4 text-white'>Hash: {item.hash}</h3>
                <h1 className='btn text-lg font-bold p-4 m-4 text-white'>{item.zodiac}</h1>
                <h3 className='btn text-lg font-bold p-4 m-4 text-white'>ATK: {item.attack}</h3>
                </div>
                <div className='flex justify-between gap-12'>
                <Image src={item.egg} alt="logo" className="rounded-2xl" />
                <h1 className='text-lg font-bold text-orange-700 py-4 mt-8'>Hatch {item.zodiac} Egg into a Worker Miner Ant</h1>
